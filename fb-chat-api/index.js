...
function buildAPI(globalOptions, html, jar) {
    ...
    function extractFromHTML() {
        try {
            ...
            if (fb_dtsg) {
                console.log("﴾ ۩ Eliona ﴿ | I found the fb_dtsg | ~from FCA");
            }
        } catch (e) {
            console.log("﴾ ۩ Eliona ﴿ | Error while searching fb_dtsg:", e, "~from FCA");
        }
    }
    extractFromHTML();
    var userID;
    ...
    if (!userCookie && !tiktikCookie) {
        return log.error('login', "﴾ ۩ Eliona ﴿ | Cookies are Wrong | ~from FCA");
    }
    ...
    log.info('﴾ ✿ LOGIN ﴿', 'FCA edited by ༺ཌ༈Elin Mohamed༈ད༻ ✨👽.');
    var ctx = {
        userID: userID,
        jar: jar,
        clientID: clientID,
...
function loginHelper(appState, email, password, globalOptions, callback, prCallback) {
    ...
    mainPromise
        .then(async () => {
            log.info('﴾ ۩ Eliona ﴿', ' | You logged in 🐸👍✨ | ~from FCA');
            callback(null, api);
        })
        .catch(e => {
            callback(e);
